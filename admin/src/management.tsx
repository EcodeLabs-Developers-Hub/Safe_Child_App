...
function StudentModal({ record, parents, teachers, classes, onClose }: { record?: RecordData; parents: RecordData[]; teachers: RecordData[]; classes: RecordData[]; onClose: () => void }) {
  ...
  const save = async () => { setBusy(true); try { const parent = parents.find((user) => String(user.email || '').toLowerCase() === form.guardianEmail.toLowerCase()); const teacher = teachers.find((user) => String(user.email || '').toLowerCase() === form.teacherEmail.toLowerCase()); const fields = { ...form, guardianName: parent?.displayName || form.guardianName, guardianEmail: parent?.email || form.guardianEmail, guardianPhone: parent?.phone || parent?.phoneNumber || parent?.mobile || form.guardianPhone, teacherName: teacher?.displayName || form.teacherName, status: 'Active' }; if (record) await updateRecord('students', record.id, fields); else await createRecord('students', fields); onClose(); } catch { setBusy(false); } };
  ...
  const selectClass = (classId: string) => { const selectedClass = classes.find((item) => item.id === classId); const teacherEmail = String(selectedClass?.teacherEmail || ''); const teacher = teachers.find((item) => String(item.email || '').toLowerCase() === teacherEmail.toLowerCase()); setForm((current) => ({ ...current, classId, className: String(selectedClass?.name || ''), grade: String(selectedClass?.grade || ''), teacherEmail, teacherName: String(selectedClass?.teacherName || teacher?.displayName || '') })); };
  const selectTeacher = (teacherEmail: string) => { const teacher = teachers.find((item) => String(item.email || '') === teacherEmail); const teacherClass = classes.find((item) => String(item.teacherEmail || '').toLowerCase() === teacherEmail.toLowerCase()); setForm((current) => ({ ...current, teacherEmail, teacherName: String(teacher?.displayName || ''), ...(teacherClass ? { classId: teacherClass.id, className: String(teacherClass.name || ''), grade: String(teacherClass.grade || '') } : {}) })); };
  return <Modal title={record ? 'Edit student and assignments' : 'Add student'} onClose={onClose}><Field label="First name" value={form.firstName} onChange={(value) => set('firstName', value)} /><Field label="Last name" value={form.lastName} onChange={(value) => set('lastName', value)} /><Field label="Grade" value={form.grade} readOnly={Boolean(form.classId)} onChange={(value) => set('grade', value)} /><label>Parent account<select value={form.guardianEmail} onChange={(event) => selectParent(event.target.value)}><option value="">Select parent</option>{parents.map((parent) => <option key={parent.id} value={String(parent.email || '')}>{String(parent.displayName || parent.email)}</option>)}</select></label><Field label="Guardian name" value={form.guardianName} readOnly={Boolean(form.guardianEmail)} onChange={(value) => set('guardianName', value)} /><Field label="Guardian email" value={form.guardianEmail} readOnly={Boolean(form.guardianEmail)} onChange={(value) => set('guardianEmail', value)} type="email" /><Field label="Guardian phone" value={form.guardianPhone} readOnly={Boolean(form.guardianEmail)} onChange={(value) => set('guardianPhone', value)} /><label>Teacher<select value={form.teacherEmail} onChange={(event) => selectTeacher(event.target.value)}><option value="">Select teacher</option>{teachers.map((teacher) => <option key={teacher.id} value={String(teacher.email || '')}>{String(teacher.displayName || teacher.email)}</option>)}</select></label><label>Grade class<select value={form.classId} onChange={(event) => selectClass(event.target.value)}><option value="">Select class</option>{classes.map((item) => <option key={item.id} value={item.id}>{String(item.name || item.grade || item.id)}{item.teacherName ? ` · ${String(item.teacherName)}` : ''}</option>)}</select></label><SaveBar busy={busy} onCancel={onClose} onSave={save} /></Modal>;
}
